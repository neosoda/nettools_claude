...
// seedV2ArubaRules ajoute les règles de conformité pour les équipements Aruba/HPE.
func seedV2ArubaRules(db *gorm.DB) {
	rules := []models.AuditRule{
		...
		{ID: "aruba-log-host", Name: "Serveur syslog distant configuré",
			Description: "Un serveur syslog distant doit être configuré",
			Pattern: `^log host \d+\.\d+\.\d+\.\d+`, MustMatch: true, Severity: "medium",
			Vendor: "aruba", Enabled: true,
			Remediation: "log host <votre-serveur-syslog>"},
		...
		{ID: "aruba-ntp-acad", Name: "Serveur NTP configuré",
			Description: "Un serveur NTP distant doit être configuré",
			Pattern: `^ntp server`, MustMatch: true, Severity: "medium",
			Vendor: "aruba", Enabled: true,
			Remediation: "ntp server <votre-serveur-ntp>"},

		// ── SNMP ──────────────────────────────────────────────────────
		{ID: "aruba-snmp-community", Name: "Communauté SNMP configurée",
			Description: "Une communauté SNMP sécurisée doit être configurée",
			Pattern: `^snmp-server community`, MustMatch: true, Severity: "high",
			Vendor: "aruba", Enabled: true,
			Remediation: "snmp-server community <votre-communaute>"},
		...
		{ID: "aruba-radius", Name: "Serveur Radius configuré",
			...
			Vendor: "aruba", Enabled: true,
			Remediation: "radius-server host <votre-serveur-radius> key <secret>"},
