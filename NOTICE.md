# NetworkTools — Notice d'utilisation
...
### Qu'est-ce que NetworkTools ?

NetworkTools est une application desktop autonome pour **Windows** destinée aux administrateurs réseau. Elle regroupe dans une seule interface les opérations les plus courantes de gestion d'infrastructure réseau :
...
Tout est conservé dans `%APPDATA%\NetworkTools\` :

```
%APPDATA%\NetworkTools\
├── networktools.db          Base de données SQLite
├── settings.json            Préférences utilisateur
├── backups\                 Fichiers de configuration sauvegardés
└── logs\
    └── networktools-YYYY-MM.log   Journaux mensuels
```
...
Le panneau gauche liste les fichiers de logs mensuels (`networktools-2025-11.log`). Cliquer sur un fichier pour afficher son contenu brut dans le panneau droit.
...
| Répertoire backup | Dossier de stockage des configs | `%APPDATA%\NetworkTools\backups` |
...
```
→ Une autre instance de NetworkTools est peut-être ouverte
...
Les logs détaillés sont dans `%APPDATA%\NetworkTools\logs\networktools-YYYY-MM.log`.

Format JSON structuré, consultable directement dans l'application (Journaux → Fichiers journaux) ou avec un éditeur texte.

---

*NetworkTools — Région Grand Est — Administration réseau*
